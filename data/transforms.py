        ...
        for k, v in target.items():
            if not isinstance(v, str):
                if torch.is_tensor(v) or isinstance(v, (list, tuple)):
                    ...
                    # v can be a numpy array (e.g., masks) or a scalar (e.g., slice index).
                    # Only copy array-like objects.
                    if isinstance(v, np.ndarray):
                        v = v.copy()
                        target[k] = torch.from_numpy(v).type(torch.LongTensor)
                    elif torch.is_tensor(v):
                        target[k] = v.clone().type(torch.LongTensor)
                    else:
                        # int/float etc.
                        target[k] = torch.tensor(v).type(torch.LongTensor)
